...
Ask the user in a single message for: project name, one-line description, primary language/framework, deployment target, GitHub org/repo, GitHub username, author/full name (for `LICENSE` + `docs/conf.py`), the Python package/import name if Python (for the `src/app` rename), contacts for conduct and security reports (one address, or two if they differ), noting that both are published publicly, so they should give addresses they are willing to publish, build/test/lint commands, license (MIT, Apache-2.0, proprietary, etc.), any version corrections for training data, and whether to use Liza (activate it by default, install its agent toolchain, or remove it — see `.devcontainer/liza/README.md`). List the installed plugin skills/agents (from `enabledPlugins` in `.claude/settings.json`) so the user can choose which to disable via `skillOverrides`.
...
- Liza: to activate it by default, change the `false` default in `${localEnv:ACTIVATE_LIZA:false}` in `.devcontainer/devcontainer.json` to `true` (the same for `INSTALL_LIZA_TOOLS` if chosen), then activate the current clone so it is live without a rebuild: `liza init --claude --yes`, or with the toolchain, `bash .devcontainer/liza/tools.sh && source ~/.liza/toolchain/env.sh && liza init --claude --yes` (init reads the tool switches from the environment). If the build/test commands differ from `make check`, update the Tier 1 rule in `GUARDRAILS.md`. To remove Liza, follow **Removing Liza** in `.devcontainer/liza/README.md`
