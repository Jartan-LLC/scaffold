...
- Liza: to activate it by default, change the `false` default in `${localEnv:ACTIVATE_LIZA:false}` in `.devcontainer/devcontainer.json` to `true` (the same for `INSTALL_LIZA_TOOLS` if chosen), then activate the current clone so it is live without a rebuild: `bash .devcontainer/liza/activate.sh`, prefixed with `INSTALL_LIZA_TOOLS=true` if the toolchain was chosen. If the build/test commands differ from `make check`, update the Tier 1 rule in `GUARDRAILS.md`. To remove Liza, follow **Removing Liza** in `.devcontainer/liza/README.md`
